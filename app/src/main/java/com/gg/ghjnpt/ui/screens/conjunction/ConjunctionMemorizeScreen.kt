package com.gg.ghjnpt.ui.screens.conjunction
...
import com.gg.ghjnpt.ui.components.ConjunctionMemorizeCard
...
@Composable
fun ConjunctionMemorizeScreen(navController: NavHostController, viewModel: QuizViewModel) {
    var revealedItems by remember { mutableStateOf(setOf<Int>()) }
    var showOnlyDifficult by remember { mutableStateOf(false) }

    val displayConjunctions = remember(showOnlyDifficult, viewModel.conjunctions, viewModel.difficultConjunctions) {
        if (showOnlyDifficult) {
            viewModel.conjunctions.filter { viewModel.isDifficultConjunction(it) }
        } else {
            viewModel.conjunctions
        }
    }
    ...
    Scaffold(
        ...
        Column(
            ...
            Row(
                ...
                Text(
                    text = if (showOnlyDifficult)
                        "어려운 접속사 (${displayConjunctions.size}개)"
                    else
                        "접속사 암기 (${displayConjunctions.size}개)",
                    fontSize = 20.sp,
                ...
                Box {
                    IconButton(
                        ...
                        Icon(
                            ...
                            contentDescription = "어려운 접속사 필터",
                            tint = if (showOnlyDifficult) YongdalBlue else Color.Gray,
                            modifier = Modifier.size(32.dp)
                        )
                    }

                    if (viewModel.difficultConjunctions.isNotEmpty()) {
                        Box(
                            modifier = Modifier
                            ...
                            Text(
                                text = "${viewModel.difficultConjunctions.size}",
                                color = Color.White,
                                fontSize = 10.sp,
                                fontWeight = FontWeight.Bold
                            )
            ...
            if (displayConjunctions.isEmpty()) {
                Box(
                    modifier = Modifier.fillMaxSize(),
                    contentAlignment = Alignment.Center
                ) {
                ...
                LazyColumn(
                    ...
                    items(
                        ...
                        ConjunctionMemorizeCard(
                            ...
                            onToggleDifficult = {
                                viewModel.toggleDifficultConjunction(conjunction)
                            }
                        )
